...
*(Path note, 2026-09-07: file paths in task descriptions were updated from the original
`js/`/`css/` layout to `src/js/`/`src/css/` when traceability checking arrived — the
project tree moved under `src/` in the Pages-deploy change, and the records now name the
files where they actually live. No task content changed.)*
...
- [X] T001 Create the project tree exactly as specified in plan.md's Project Structure: `index.html`, `src/css/styles.css`, `src/js/theory.js`, `src/js/state.js`, `src/js/audio.js`, `src/js/fretboard.js`, `src/js/controls.js`, `src/js/main.js`, `tests/theory.test.js` (empty/stub files)
...
- [X] T003 [P] Create `src/css/styles.css` skeleton: CSS custom properties for the 12 chromatic degree-role color pairs (bright + dark variant each, per data-model.md's Chromatic Degree Role), base page layout, no component styling yet
- [X] T004 [P] Confirm `node --test` on `tests/theory.test.js` runs cleanly against the empty stub (zero tests, zero failures) — establishes the hard-gate test command works before any theory.js work begins

---

## Phase 2: Foundational — `src/js/theory.js` (BLOCKS all user stories)

**Purpose**: The single canonical, pure, dependency-free music-theory module (constitution Principle I). Every user story phase below calls into this module — none may duplicate its math. Every function is built test-first: **write the test in `tests/theory.test.js`, confirm it fails, then implement in `src/js/theory.js`.** No task here is folded into a generic "implement theory.js" line — each function (and the capo/Relative-mode binding rule) gets its own explicit, independently-checkable test task and implementation task, per contracts/theory-api.md.
...
- [X] T006 Implement the `TUNINGS` export in `src/js/theory.js` to satisfy T005 (depends on T005 failing first)
...
- [X] T008 Implement the `SCALES` export in `src/js/theory.js` to satisfy T007 (depends on T007)
...
- [X] T010 Implement the `DEGREE_ROLES` export in `src/js/theory.js` to satisfy T009 (depends on T009)
...
- [X] T012 Implement `noteAt()` in `src/js/theory.js` to satisfy T011 (depends on T011, T006)
...
- [X] T014 Implement `spellPitchClass()` in `src/js/theory.js` to satisfy T013 (depends on T013)
...
- [X] T016 Implement `getDiatonicSemitones()` in `src/js/theory.js` to satisfy T015 (depends on T015, T008)
...
- [X] T018 Implement `getDegreeRole()` in `src/js/theory.js` to satisfy T017 (depends on T017, T010)
...
- [X] T020 Implement `getDegreeLabel()` in `src/js/theory.js` to satisfy T019 (depends on T019, T008)
...
- [X] T022 Implement `getIntervalLabel()` in `src/js/theory.js` to satisfy T021 (depends on T021)
...
- [X] T024 Implement `computeDefaultTriad()` in `src/js/theory.js` to satisfy T023 (depends on T023, T016)
...
- [X] T026 Implement `getTriadQuality()` in `src/js/theory.js` to satisfy T025 (depends on T025)
...
- [X] T028 Implement `isToggleableChordTone()` in `src/js/theory.js` to satisfy T027 (depends on T027, T016)
...
- [X] T030 Implement `identifyChordQuality()` in `src/js/theory.js` to satisfy T029 (depends on T029)
...
- [X] T032 Implement `getDisplayRootSemitone()` in `src/js/theory.js` to satisfy T031 (depends on T031)
...
- [X] T034 Implement `getRelativeLabelSemitone()` in `src/js/theory.js` to satisfy T033 (depends on T033)
- [X] T035 Write unit tests for `isFretPlayable(fret, capoFret)` in `tests/theory.test.js`: `fret >= capoFret` across boundary values
- [X] T036 Implement `isFretPlayable()` in `src/js/theory.js` to satisfy T035 (depends on T035)
...
- [X] T038 Run `node --test` on `tests/theory.test.js`: confirm 100% pass across every function above (T005–T037), satisfying constitution Principle I's hard gate — no later-phase task may begin until this passes

---

## Phase 2b: Foundational — `src/js/state.js` + `src/js/main.js` bootstrap (BLOCKS all user stories)
...
- [X] T042 [US1] Build the inline SVG fretboard skeleton in `src/js/fretboard.js`: one `<svg>` with one `<g>` per string×fret position (6 strings × 25 frets), string 1 (high-E) top row → string 6 (low-E) bottom row, stable `id`/`data-string`/`data-fret` attributes, using `noteAt()` (T012) for each position's pitch
- [X] T043 [US1] Render inlay fret markers in `src/js/fretboard.js`: single-dot at frets 3,5,7,9,15,17,19,21; double-dot at 12 and 24 (depends on T042)
- [X] T044 [US1] Render open-string notes distinctly, positioned left of fret 1 past the nut line, in `src/js/fretboard.js` (depends on T042)
- [X] T045 [US1] Render the persistent base note-name label (via `spellPitchClass()`, T014) at every position, in a neutral non-distracting style, in `src/js/fretboard.js` (depends on T042)
- [X] T046 [US1] Wire `src/js/main.js` to call `fretboard.render(state)` once after `state.load()`, using the default/restored state — first visible render (depends on T041, T042, T043, T044, T045)
- [X] T047 [US1] Add per-note `aria-label`s (e.g. `"E4, root, fret 0, string 1"`) and keyboard focusability to every SVG `<g>` in `src/js/fretboard.js` (depends on T042)
- [X] T048 [P] [US1] Style `src/css/styles.css`: base layer typography/contrast (WCAG AA), fret-marker dots, open-string visual distinction, nut line
...
- [X] T050 [US2] Build the tuning selector (D-Family/G-Family/C-Family groups + Standard) in `index.html`/`src/js/controls.js`, sourced from `theory.TUNINGS` (T006)
- [X] T051 [US2] Build Custom Tuning per-string pitch inputs in `src/js/controls.js` (depends on T050)
...
- [X] T053 [P] [US2] Style the tuning selector and custom-tuning inputs in `src/css/styles.css`
...
- [X] T055 [US3] Build the root-note selector (C D E F G A B) in `src/js/controls.js`
- [X] T056 [US3] Build the sharp/flat enharmonic toggle, enabled only for roots with an enharmonic pair (Edge Case: disabled for natural-only roots), wired to `state.accidentalPreference`, in `src/js/controls.js` (depends on T055)
- [X] T057 [US3] Wire root/accidental changes → `fretboard.render(state)`: root highlight (`isRoot`) and relabeling via `spellPitchClass()` (T014), in `src/js/fretboard.js` (depends on T056, T042)
...
- [X] T059 [US4] Build the scale/mode selector, grouped Church Modes/Pentatonic/Blues/Other, in `src/js/controls.js`, sourced from `theory.SCALES` (T008)
- [X] T060 [US4] Compute `Note.isDiatonic` per position via `getDiatonicSemitones()` (T016) in `src/js/fretboard.js`'s render pipeline; diatonic notes get a color cue, non-diatonic notes get none (base layer only) (depends on T059, T057)
- [X] T061 [US4] Confirm scale/mode change clears prior highlighting immediately — no residual DOM classes from the previous scale — in `src/js/fretboard.js` (depends on T060)
...
- [X] T063 [US5] Assign each note's fixed `degreeRole` via `getDegreeRole()` (T018) and render the dark color-role variant for diatonic notes in `src/js/fretboard.js` (depends on T060)
- [X] T064 [US5] Implement focal-point click handling in `src/js/fretboard.js`/`src/js/controls.js`: only diatonically-colored notes are clickable as focal, default focal = root (semitone 0) on every root/scale change (depends on T063)
- [X] T065 [US5] Compute the default triad via `computeDefaultTriad()` (T024) and its quality via `getTriadQuality()` (T026); render focal-triad notes bright + bordered, other diatonic notes dark, in `src/js/fretboard.js` (depends on T064)
- [X] T066 [US5] Implement the chord-tone override toggle UI, gated by `isToggleableChordTone()` (T028), in `src/js/controls.js` (depends on T065)
- [X] T067 [US5] Display the active bright note set + recognized chord-quality label via `identifyChordQuality()` (T030) in `index.html`/`src/js/controls.js` (depends on T066)
- [X] T068 [P] [US5] Implement focal-point + `chordToneOverrides` reset on root/scale change (Edge Case) in `src/js/state.js` (depends on T039)
- [X] T069 [P] [US5] Style bright/dark/bordered variants for all 12 color roles in `src/css/styles.css`
...
- [X] T071 [US6] Build the label-mode selector (Notes/Degrees/Intervals) in `src/js/controls.js`
- [X] T072 [US6] Compute `displayLabel` per active mode in `src/js/fretboard.js`: Notes via `spellPitchClass()` (T014), Degrees via `getDegreeLabel()` (T020), Intervals via `getIntervalLabel()` (T022); non-diatonic notes always show their letter name (depends on T071, T063)
- [X] T073 [US6] Confirm the root is distinguished by color-role + secondary border/shape indicator in every label mode, in `src/js/fretboard.js` (depends on T072)
...
- [X] T075 [US7] Build the dual-handle slider markup in `index.html` + `src/css/styles.css`
- [X] T076 [US7] Implement drag handling + constraints (left ≤ right, minimum 1 fret visible) in `src/js/controls.js`, labeling the left handle `"N"` at the nut or the fret number otherwise, and the right handle always with its fret number (depends on T075)
- [X] T077 [P] [US7] Wire `fretRange` state (`lowerBound`/`upperBound`, default `0`/`24`) in `src/js/state.js` (depends on T039)
- [X] T078 [US7] Hide/show frets outside `[lowerBound, upperBound]` in `src/js/fretboard.js` — visibility only, no recomputation of pitch/diatonic/focal data (depends on T075, T076, T077, T042)
- [X] T078a [US7] Build a reset control (button near the slider) in `index.html`/`src/js/controls.js` that resets both handles to their default positions (left="N", right=24) and updates `fretRange` state accordingly (FR-027), regardless of the current range (depends on T075, T076, T077)
- [X] T079 [US7] Manually validate against quickstart.md Section 6 (default N–24; drag left to 5; drag right to 12; combined range; drag-past-other-handle constraint; reset control returns to N–24 from any adjusted range)
...
- [X] T080 [US8] Load `soundfont-player` + the `acoustic_guitar_steel` instrument via the `gleitz/midi-js-soundfonts` CDN, caching the loaded instrument, in `src/js/audio.js`
- [X] T081 [US8] Implement `audio.play(midiNote)` in `src/js/audio.js`, using `noteAt()`'s `midiNote` (T012) — always the true sounding pitch, never a label-mode-adjusted one (depends on T080)
- [X] T082 [US8] Create/resume the `AudioContext` inside the first user-gesture handler only — no autoplay on load or state change — in `src/js/audio.js` (depends on T080)
- [X] T083 [US8] Wire fret click/tap → `audio.play()` in `src/js/controls.js`/`src/js/fretboard.js` (depends on T081, T082, T042)
- [X] T084 [US8] Verify/implement clean handling of rapid sequential triggers (no improper cutoff, no audible lag) in `src/js/audio.js` (depends on T081)
- [X] T085 [US8] Implement a visible, non-blocking error toast/banner on sample load failure, with retry-on-next-tap for the affected note (FR-041), in `src/js/audio.js` + `index.html`/`src/css/styles.css` (depends on T080)
...
- [X] T087 [US9] Build the capo fret selector (0–12) in `src/js/controls.js`
- [X] T088 [US9] Compute `isPlayable` per note via `isFretPlayable()` (T036); mute/hide frets `< capoFret`, in `src/js/fretboard.js` (depends on T087, T042)
- [X] T089 [US9] Lock the fret-range left handle to `capoFret` with a `"Capo"` label when `capoFret>0`; release it to free movement/`"N"` when `capoFret===0`; move the right handle up if it would fall below `capoFret` (FR-035, FR-036), in `src/js/controls.js` (depends on T076, T088)
- [X] T090 [US9] Build the Absolute/Relative label-mode toggle, wired to `state.capoLabelMode`, in `src/js/controls.js` (depends on T087)
- [X] T091 [US9] Implement Relative-mode note-NAME computation in `src/js/fretboard.js`: `spellPitchClass(tuning.openPitchClasses[stringIndex]'s semitone + getRelativeLabelSemitone(fret, capoFret), keyContext)` (T034) — using the tuning's static open pitch class directly, never a capo-raised pitch, per contracts/theory-api.md's "Note-name resolution in Relative mode" (depends on T090, T034, T014)
- [X] T092 [P] [US9] Verify `audio.play()` continues to use `noteAt()`'s Absolute `midiNote` regardless of `capoLabelMode` (FR-032) in `src/js/audio.js` — confirm no capo-relative branch was accidentally introduced in T081/T083
- [X] T093 [US9] **Implement the `getDisplayRootSemitone` binding-rule wiring** in `src/js/fretboard.js`: on every render, compute `const displayRootSemitone = theory.getDisplayRootSemitone(trueRootSemitone, state.capoFret, state.capoLabelMode)`, and pass its output — **never the raw Story-3-selected root** — as the `root` argument to `getDiatonicSemitones()` (T016), `computeDefaultTriad()` (T024), `isToggleableChordTone()` (T028), and `identifyChordQuality()` (T030) on every render pass, per contracts/theory-api.md's binding rule and spec Story 9 Acceptance Scenario 7 (depends on T032, T016, T024, T028, T030, T060, T065, T066, T067)
- [X] T094 [US9] **Verify the binding-rule wiring** manually against quickstart.md Section 8 step 3 and spec Story 9 Acceptance Scenario 7: with a key/scale selected and a capo active, toggling Absolute↔Relative visibly changes the diatonic *color-highlighted note set* (not merely label text); confirm at `capoFret=0` the highlighted set is identical in both modes (depends on T093)
- [X] T095 [US9] Manually validate the remaining capo scenarios against quickstart.md Section 8 (frets muted below capo; Absolute "C" vs. Relative "A" at capo-3 on the A string; capo reset to 0 releases the left handle to "N")
...
- [X] T097 Full accessibility pass across `index.html`/`src/css/styles.css`/`src/js/fretboard.js`: keyboard navigability for every control and fret cell, WCAG AA contrast in every highlight state, and a colorblind-simulation check that shape/border alone conveys every color-coded state (SC-003)
- [X] T098 Handle remaining Edge Cases in `src/js/fretboard.js`/`src/js/audio.js`: custom tuning with duplicate/unplayably-extreme pitches still renders via raw pitch math with no rejection; a note triggered mid-tuning-change finishes playing before the new tuning applies to subsequent triggers
...
- [X] T102 [US9] Implement `getHighlightRootSemitone()` in `src/js/theory.js` to satisfy T101 (depends on T101)
...
- [X] T105 [US9] Run `node --test` on `tests/theory.test.js`: confirm T101–T104 pass — hard gate before any rewiring below (depends on T101, T102, T103, T104)
- [X] T106 [US9] Rewire `src/js/fretboard.js`'s render pipeline: replace the `getDisplayRootSemitone` call from T093 with `theory.getHighlightRootSemitone(trueRootSemitone, state.capoFret, state.capoLabelMode)`, feeding its output — never the raw Story-3-selected root — as the `root` argument to `getDiatonicSemitones()`, `computeDefaultTriad()`, `isToggleableChordTone()`, and `identifyChordQuality()` on every render pass (depends on T105)
- [X] T107 [US9] Add a second, independent `computeDefaultTriad()`/`identifyChordQuality()` call in `src/js/fretboard.js`/`src/js/controls.js` for the "Bright notes: X, Y, Z (Quality)" text summary (T067), fed the TRUE root semitone always — never `getHighlightRootSemitone`'s output — so the displayed text stays anchored to the true root even when on-fretboard highlighting is shifted (FR-048) (depends on T106, T067)
...
- [X] T111 [US1] Fix `src/js/fretboard.js`'s marker-dot rendering (T043) to key strictly off true physical fret position, independent of any Relative-mode renumbering logic (FR-049) — root-cause the current failure before patching, per the source doc's "do not guess" instruction (depends on T110)
...
- [X] T113 [P] [US9] Add a new `--color-capo-indicator` custom property to `src/css/styles.css`'s `:root` block, distinct from whatever token/value styles the true-nut line (FR-050)
- [X] T114 [US9] Render a capo position indicator (vertical line/bar) in `src/js/fretboard.js` at `capoFret`'s column whenever `capoFret>0`, styled via `var(--color-capo-indicator)` with a color and/or thickness distinct from the true-nut indicator (depends on T113, T088)
...
- [X] T118 [US3] Replace `.root-buttons button[aria-pressed="true"]`'s hardcoded `#ffd54a` background/border in `src/css/styles.css` with the same token used for the fretboard's root-marker color-role (depends on T117)
...
- [X] T120 [P] Add UI-accent design tokens to `src/css/styles.css`'s `:root` block per data-model.md's Design Tokens section: `--color-root-accent` (the `#ffd54a` gold currently duplicated across the root-marker border, bright-root marker border, and fret-range-thumb focus outline — EXCLUDING `.root-buttons button[aria-pressed="true"]`, whose value is intentionally changing per T118/FR-051, not being preserved as this accent), `--color-text-on-bright` (the `#14171c` currently duplicated across several `is-bright`/`aria-pressed="true"` rules — may simply alias `var(--color-bg)`, which already holds this exact value), and `--color-error-bg`/`--color-error-text` (the audio-error-banner's `#7a2323`/`#fff` pair) (FR-052)
- [X] T121 Replace every raw hex literal identified in T120 with its new `var(--token-name)` reference across `src/css/styles.css`, confirming via visual diff that no rendered color value changes anywhere (FR-052) (depends on T120)
- [X] T122 Manually validate: change one token's value in `:root` and confirm every rule referencing it updates consistently, with no remaining duplicate hardcoded literal for that color anywhere in `src/css/styles.css` (depends on T121)
...
- [X] T124 [P] Style the new link in `src/css/styles.css` consistently with the existing credit-line fonts/colors/spacing — no new visual pattern introduced (depends on T123)
...
No task in `src/js/fretboard.js`, `src/js/audio.js`, or `src/js/controls.js` (Phases 3–11) may be started before the specific `src/js/theory.js` function(s) it cites as a dependency have both a passing test (odd-numbered T-task) and an implementation (its paired even-numbered T-task) from Phase 2. Every task above that calls into `theory.js` names the exact T-task IDs it depends on for this reason — this is enforced explicitly per-task, not left to phase ordering alone.
...
Phase 2's checkpoint (T038: `node --test` on `tests/theory.test.js` green) is a **hard, non-negotiable gate** per constitution Principle I/IV. It is not a "nice to have before Phase 3" — no later-phase task may be started until it passes, including the four functions named in the `getDisplayRootSemitone` binding rule (T037), which itself must pass before T093 (the binding-rule wiring in `fretboard.js`) may be written.

---

## Notes

- [P] tasks touch different files with no dependency on an incomplete task in the same phase — this excludes most of Phase 2, since nearly every theory task shares `src/js/theory.js` and `tests/theory.test.js`.
...
- T101–T105 MUST pass (`node --test` on `tests/theory.test.js` green) before T106 is written — this is the same test-before-implementation gate the constitution applies to every other `theory.js` function, applied here to the corrected capo/Relative rule specifically.
- T106 supersedes T093's wiring in `src/js/fretboard.js` at the code level (calls `getHighlightRootSemitone` instead of the now-superseded `getDisplayRootSemitone`) but T093/T094 themselves are left as-is per instruction — they document what round-1 built, not what's currently wired.
...
- [P] tasks in Phase 13 touch different files (or different, independent regions of `src/css/styles.css`) from any other incomplete task in the same section.

## Traceability Retrofit (2026-09-07)

Records the automated test coverage that exists for each User Story's acceptance
criteria — written during UAT rounds and the constitution v1.2.0 coverage close-out,
but never logged as tasks. Added so every plan item carries a test task (spec-trace T2).

- [X] T126 [US1] Test task for US-1.1: jsdom suites in `tests/fretboard.test.js` and `tests/styles.test.js` cover the fretboard layout scenarios (string order, inlay dots, open strings, color+shape distinction, dot anchoring under capo/Relative)
- [X] T127 [US2] Test task for US-1.2: suites in `tests/theory.test.js` (TUNINGS data, noteAt per tuning), `tests/controls.test.js` (tuning selector, custom tuning), and `tests/state.test.js` (tuning state) cover the tuning scenarios
- [X] T128 [US3] Test task for US-1.3: suites in `tests/theory.test.js` (spellPitchClass, circle-of-fifths spelling), `tests/controls.test.js` (root selector, selected-root button color), and `tests/styles.test.js` (root color-role token) cover the root-selection scenarios
- [X] T129 [US4] Test task for US-1.4: suites in `tests/theory.test.js` (SCALES data, getDiatonicSemitones, getDegreeLabel) and `tests/fretboard.test.js` (highlight rendering, stale-highlight clearing) cover the scale/mode scenarios
- [X] T130 [US5] Test task for US-1.5: suites in `tests/theory.test.js` (computeDefaultTriad, getTriadQuality, isToggleableChordTone, identifyChordQuality), `tests/fretboard.test.js` (bright/dark/bordered rendering, focal clicks), and `tests/controls.test.js` (chord-tone toggles, bright-set display) cover the focal-point scenarios
- [X] T131 [US6] Test task for US-1.6: suites in `tests/theory.test.js` (getDegreeLabel, getIntervalLabel) and `tests/fretboard.test.js` (base layer, per-mode labels, root distinction) cover the label-display scenarios
